...
void colla_filter(vector<vector<int> >& matrix_test5, vector<vector<int> >& matrix_reshape_train, vector<vector<int> >& matrix_reshape_test5, int userID)
{
    ...
    for(int i=0; i<matrix_reshape_train.size(); i++){
        //cout<<"train user "<<matrix_reshape_train[i][0]<<endl;
        double ra_sum = 0.0;
        double ru_sum = 0.0;
        double count = 0.0;
        //cout<<"train user: "<<matrix_reshape_train[i][0]<<endl;
        for(int j=0; j<movie_k.size(); j++){
            //cout<<"at movie "<<movie_k[j]<<endl;
            if(matrix_reshape_train[i][movie_k[j]]==0){
                continue;
            }
            //cout<<"ru_sum = "<<ru_sum<<" + "<<matrix_reshape_train[i][movie_k[j]]<<endl;
            ru_sum = ru_sum + matrix_reshape_train[i][movie_k[j]];
            //cout<<"ra_sum = "<<ra_sum<<" + "<<rate_k[j]<<endl;
        ...
        if(count==0.0||count == 1){
            ...
            for(int k=0; k<movie_k.size(); k++){
                ...
                //cout<<"ra_sig = "<<ra_sig<<" + ("<<rate_k[k]<<" - "<<ra_sum_avg<<")^2"<<endl;
            ...
            if(ru_sig == 0.0||ra_sig == 0.0){
                weight = 0.0;
                //cout<<"sine ru_sig = 0 or ra_sig = 0, weight = 0"<<endl;
            } else {
    ...
    for(int i=0; i<sorted_cos_sim.size(); i++){
        cout<<"sorted user: "<<sorted_cos_sim_user[i]<<" sim: "<<sorted_cos_sim[i]<<endl;
    }
    ...
    ofstream file("result5_pi.txt", ios::app);
    if(file.is_open()){
