...
void colla_filter(vector<vector<int> >& matrix_test5, vector<vector<int> >& matrix_reshape_train, vector<vector<int> >& matrix_reshape_test5, int userID)
{
    ...
    vector<vector<int> > matrix_train_iuf;
    double iuf;
    matrix_train_iuf.resize(200,vector<int> (1001));
    int m_train = matrix_reshape_train.size();
    int m_test = matrix_reshape_test5.size();
    ...
    for(int i=0; i<matrix_reshape_train.size(); i++){
        ...
        double ru_sum = 0.0;
        double count = 0.0;
        double ra_sum_iuf = 0.0;
        double ru_sum_iuf = 0.0;
        for(int j=0; j<movie_k.size(); j++){
            double timer = 0.0;
            double t = 0.0;
            for(int u=0; u<200; u++){
                if(matrix_reshape_train[u][movie_k[j]]!=0){
                    timer ++;
                }
            }
            for(int a = 0; a<100; a++){
                if(matrix_reshape_test5[a][movie_k[j]]!=0){
                    t ++;
                }
            }
            double mj_train = timer;
            double mj_test = t;
            if(matrix_reshape_train[i][movie_k[j]]==0){
                continue;
            }
            double iuf_train = log(m_train/mj_train);
            double iuf_test = log(m_test/mj_test);
            ru_sum = ru_sum + matrix_reshape_train[i][movie_k[j]];
            ra_sum = ra_sum + rate_k[j];
            ru_sum_iuf = ru_sum_iuf + matrix_reshape_train[i][movie_k[j]]*iuf_train;
            ra_sum_iuf = ra_sum_iuf + rate_k[j]*iuf_test;
            count ++;
        }
        ...
        double weight;
        double weight_end;
        double ra_avg_iuf = 0.0;
        double ru_avg_iuf = 0.0;
        if(count==0.0||count == 1){
            ...
            ra_avg_iuf = ra_sum_iuf/count;
            ru_avg_iuf = ru_sum_iuf/count;
            user_avg[matrix_reshape_train[i][0]] = ru_sum_avg;
            for(int k=0; k<movie_k.size(); k++){
                if(matrix_reshape_train[i][movie_k[k]]==0){
                    continue;
                }
                ra_sig = ra_sig + pow(rate_k[k]-ra_avg_iuf,2);
                ru_sig = ru_sig + pow(matrix_reshape_train[i][movie_k[k]]-ru_avg_iuf,2);
                raru_sig = raru_sig + (rate_k[k]-ra_avg_iuf)*(matrix_reshape_train[i][movie_k[k]]-ru_avg_iuf);
            }
    ...
    ofstream file("result20_p.txt", ios::app);
    if(file.is_open()){
