...
vector<vector<int> >reshape(vector<vector<int> >& matrix){
    ...
    // Resize matrix_reshape to handle the reshape
...
void colla_filter(vector<vector<int> >& matrix_test5, vector<vector<int> >& matrix_reshape_train, vector<vector<int> >& matrix_reshape_test5, int userID)
{
    ...
    for(int i=0; i<matrix_reshape_train.size(); i++){
        ...
        for(int j=0; j<movie_k.size(); j++){
            ...
            counting ++; // count how many train user rate that movie
            //cosine similarity
        ...
        double cosine;
        //if only one or no user in train rate that movie, cosine = 0 because only 1 data is not accurate
        if(counting == 1||mag_b==0.0){
            ...
            cosine = dot/(mag_a_sqrt*mag_b_sqrt);
        }
        //cos_sim_user collects user number in train
        //cos_sim collects the uesr's similarity 
    ...
    //rank the cosine similarity and corresponding user
    ...
    for(int i=0; i<sorted_movie.size(); i++){
         if(sorted_score[i]==0){
            ...
            for(int j=0; j<k; j++){
                ...
                if(rating == 0){
                    //if k user doesn't rate this movie, pass
                    continue;
                }
    ...
    ofstream file("result20_a.txt", ios::app);
    if(file.is_open()){
